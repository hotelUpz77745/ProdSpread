...
> [!NOTE]
> **СТАТУС ТЕКУЩЕЙ АРХИТЕКТУРЫ: POSTPONED / FROZEN (v11.1)**
> Проект приостановлен до неопределенного времени. Все критические находки, доработки и ТЗ для будущей разморозки задокументированы:
> 1. **Нейтрализация Мины №1 (Запрет Кейса В / Falling Knives):** Полный запрет входа в локальные проливы и ликвидации на ведомой бирже при стоячем Binance (`static_leg: "TARGET"`).
> 2. **Нейтрализация Мины №2 (Synthetic Exit 30% Cap):** Защита от «дырявых» стаканов органически возложена на `synthetic_exit` (`max_slippage_ratio: 0.30`, `hard_max_slippage: 0.008`).
> 3. **Нейтрализация Мины №3 (4-стадийный Orderbook Hunting Exit):** Отказ от слепого рыночного сброса в пользу `LIMIT_IOC` (`Base Virtual TP` $\to$ `Breakeven` $\to$ `10-Attempt Progressive Step Extrime Close` $\to$ `Guaranteed Emergency MARKET Sweep`).
> 4. **Прецизионное проскальзывание входа (`limit_slip_ratio: 0.0020` / 0.20%):** Оптимизированный лимитный заброс ордера `LIMIT_IOC` для забора объемов на альтах.
> 5. **ТЗ ДЛЯ БУДУЩЕЙ РАЗМОРОЗКИ (Привязка к вектору импульса):** Входное направление (`LONG`/`SHORT`) должно принудительно задаваться **знаком вектора импульса Оракула** ($\text{sign}(\Delta P_{oracle})$): при пампах Binance открывать только `LONG` (покупка догоняющего ралли), при дампах — только `SHORT`. Контртрендовые удары должны блокироваться.
