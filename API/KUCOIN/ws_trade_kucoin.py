...
# ROLE: Reactive trade client for Kucoin (Pro WS API v1 / UTA with resilient fallback)
# ============================================================
...
class KucoinWsTrader:
    """
    High-speed reactive trade client for Kucoin Futures.
    Supports:
    1) Pro WebSocket (wss://wsapi.kucoin.com/v1/private) with signature authentication and auto-ping.
    2) Seamless fallback to hot persistent HTTP session for Classic Futures accounts.
    """
    ...
    async def start(self):
        ...
        try:
            ...
            log("[KucoinWsTrader] Pro WebSocket stream connected and active.", level="INFO")
        except asyncio.TimeoutError:
            log("[KucoinWsTrader] WS initializing, hot REST keep-alive transport enabled.", level="INFO")
    ...
    async def _connection_loop(self):
        ...
        while self._running:
            try:
                ...
                log("[KucoinWsTrader] Connecting to Pro WS...", level="DEBUG")
                    ...
                    async for msg in ws:
                        if msg.type == aiohttp.WSMsgType.TEXT:
                            try:
                                ...
                                log(f"[KucoinWsTrader] Parse error: {parse_err}", level="WARNING")
                        elif msg.type in (aiohttp.WSMsgType.CLOSED, aiohttp.WSMsgType.ERROR):
                            break
            except asyncio.CancelledError:
                break
            except Exception as e:
                # If Classic Futures account lacks Pro WS URL access, stay on fast HTTP keep-alive
                self.ws_active = False
    ...
    async def place_order(self, symbol: str, side: str, qty_str: str, position_side: Optional[str] = None, timeout: float = 3.0) -> dict:
        """
        Sends MARKET order via reactive Pro WS or hot persistent REST transport.
        """
        req_id = str(uuid.uuid4())
        leverage = str(self.margin_settings.get("leverage", 20))
        margin_mode = str(self.margin_settings.get("margin_type", "CROSS")).upper()

        # Attempt send via Pro WS (if supported by account)
        ...
        # Reactive fallback via hot keep-alive REST session
