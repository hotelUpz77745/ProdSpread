# ============================================================
# FILE: live_tests/test_pair_hedge_binance_bitget.py
# ROLE: Тестовый замер связки BINANCE-BITGET:
...
#      11. Подробный лог в консоль и файл logs/test_hedge_binance_bitget.log
# ============================================================
import asyncio
import os
import sys
...
from API.orders import BinanceOrder, BitgetOrder

LOG_FILE = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "logs", "test_hedge_binance_bitget.log")
...
async def main():
    log_msg("=" * 75)
    log_msg("🚀 СТАРТ ТЕСТОВОГО ПРОГОНА ХЕДЖ-СВЯЗКИ: BINANCE <-> BITGET")
    log_msg("=" * 75)
    ...
    bg_stream = BitgetPositionStream(os.environ["BITGET_API_KEY"], os.environ["BITGET_API_SECRET"], os.environ["BITGET_API_PASSPHRASE"])
    ...
    def make_handler(ex):
        def cb(d):
            books[ex] = {
                "bids": [(p, q) for p, q in d.bids] if d.bids else [],
                "asks": [(p, q) for p, q in d.asks] if d.asks else []
            }
        return cb

    b_stakan = BinanceStakanStream([SYM_BINANCE])
    bg_stakan = BitgetStakanStream([SYM_BITGET])
    t_bs = asyncio.create_task(b_stakan.run(make_handler("BINANCE")))
    t_bgs = asyncio.create_task(bg_stakan.run(make_handler("BITGET")))

    log_msg("[2/5] Ожидание поступления стаканов...")
    for _ in range(50):
        if books["BINANCE"].get("asks") and books["BITGET"].get("bids"):
            break
        await asyncio.sleep(0.1)

    b_ask0 = books["BINANCE"]["asks"][0][0]
    bg_bid0 = books["BITGET"]["bids"][0][0]
    pre_spread_gross = (bg_bid0 - b_ask0) / b_ask0
    taker_fee_b = float(cfg["trading_risks"]["binance"]["taker_fee"])
    taker_fee_bg = float(cfg["trading_risks"]["bitget"]["taker_fee"])
    entry_comm = taker_fee_b + taker_fee_bg
    pre_spread_net = pre_spread_gross - entry_comm

    log_msg(f"📊 [СПРЕД ПЕРЕД ВХОДОМ]:")
    log_msg(f"   Binance Ask: {b_ask0:.5f} | Bitget Bid: {bg_bid0:.5f}")
    log_msg(f"   Gross Spread: {pre_spread_gross*100:+.4f}% | Net Spread: {pre_spread_net*100:+.4f}%")

    # 4. Вход хеджированными ордерами: LONG Binance, SHORT Bitget
    log_msg("\n[3/5] ⚡ ОТПРАВКА ОРДЕРОВ ВХОДА (LONG BINANCE | SHORT BITGET)...")
    ...
    task_bg_open = asyncio.create_task(bg_order.place_order(SYM_BITGET, "SELL", ORDER_SIZE_USD, bg_bid0, order_type="MARKET", position_side="SHORT"))

    res_b_open, res_bg_open = await asyncio.gather(task_b_open, task_bg_open, return_exceptions=True)
    ...
    log_msg(f"   Bitget Open Response:  {res_bg_open if not isinstance(res_bg_open, Exception) else f'ERROR: {res_bg_open}'}")
    ...
    for _ in range(100):
        ...
        pos_bg = bg_order.get_executed_position(SYM_BITGET, "SHORT")
        if pos_b.get("size", 0.0) > 0 and pos_bg.get("size", 0.0) > 0:
            size_long = pos_b["size"]
            size_short = pos_bg["size"]
            p_long_entry = pos_b.get("price", 0.0) or b_ask0
            p_short_entry = pos_bg.get("price", 0.0) or bg_bid0
            break
        await asyncio.sleep(0.005)

    t_poll_ms = (time.perf_counter() - t_poll_start) * 1000.0
    log_msg(f"⏱  [ПОДТВЕРЖДЕНИЕ НАЛИВА ПО WS]: {t_poll_ms:.2f} мс")
    log_msg(f"   Налито: Binance Long = {size_long} шт @ {p_long_entry:.5f}, Bitget Short = {size_short} шт @ {p_short_entry:.5f}")
    ...
    bg_ask_exit = books["BITGET"]["asks"][0][0] if books["BITGET"].get("asks") else bg_bid0
    pre_exit_gross = (bg_ask_exit - b_bid_exit) / b_bid_exit
    log_msg(f"📊 [СПРЕД ПЕРЕД ВЫХОДОМ (ПО СТАКАНУ)]:")
    log_msg(f"   Binance Bid: {b_bid_exit:.5f} | Bitget Ask: {bg_ask_exit:.5f}")
    log_msg(f"   Gross Exit Spread: {pre_exit_gross*100:+.4f}%")

    # 6. Выход хеджированными ордерами: SELL Binance, BUY Bitget
    log_msg("\n[4/5] ⚡ ОТПРАВКА ОРДЕРОВ ЗАКРЫТИЯ (SELL BINANCE | BUY BITGET)...")
    t_close_start = time.perf_counter()

    size_b_usd = size_long * b_bid_exit if size_long > 0 else ORDER_SIZE_USD
    size_bg_usd = size_short * bg_ask_exit if size_short > 0 else ORDER_SIZE_USD
    ...
    task_bg_close = asyncio.create_task(bg_order.place_order(SYM_BITGET, "BUY", size_bg_usd, bg_ask_exit, order_type="MARKET", position_side="SHORT"))

    res_b_close, res_bg_close = await asyncio.gather(task_b_close, task_bg_close, return_exceptions=True)
    ...
    log_msg(f"   Bitget Close Response:  {res_bg_close if not isinstance(res_bg_close, Exception) else f'ERROR: {res_bg_close}'}")
    ...
    for _ in range(100):
        ...
        if hasattr(bg_order, "get_last_close_price"):
            close_p_bg = bg_order.get_last_close_price(SYM_BITGET)
        if pos_b.get("size", 0.0) == 0.0 and pos_bg.get("size", 0.0) == 0.0:
            break
        await asyncio.sleep(0.005)
    ...
    if not close_p_bg:
        close_p_bg = bg_ask_exit

    log_msg(f"📊 [ФАКТИЧЕСКИЕ ЦЕНЫ ЗАКРЫТИЯ (ПО СТРИМАМ)]:")
    log_msg(f"   Binance Long Close: {close_p_b:.5f} | Bitget Short Close: {close_p_bg:.5f}")

    pnl_b = (close_p_b - p_long_entry) / p_long_entry if p_long_entry > 0 else 0.0
    pnl_bg = (p_short_entry - close_p_bg) / p_short_entry if p_short_entry > 0 else 0.0
    total_fee = entry_comm * 2.0
    net_pnl = pnl_b + pnl_bg - total_fee

    log_msg(f"\n[5/5] 🏁 ФИНАЛЬНЫЙ РЕЗУЛЬТАТ СДЕЛКИ:")
    log_msg(f"   PnL Binance (Long):  {pnl_b*100:+.4f}%")
    log_msg(f"   PnL Bitget (Short):  {pnl_bg*100:+.4f}%")
    ...
    rest_bg = await bg_order.get_exact_position_guarded(SYM_BITGET, "SHORT")
    log_msg(f"   Остаток на бирже (REST): Binance={rest_b.get('size')}, Bitget={rest_bg.get('size')}")

    # Завершение
    await b_stakan.aclose()
    await bg_stakan.aclose()
    await b_stream.stop()
    await bg_stream.stop()
    t_bs.cancel()
    t_bgs.cancel()
    t_b_stream.cancel()
    t_bg_stream.cancel()
