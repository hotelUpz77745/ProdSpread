...
#### 2. Математика расчета стояка (Top-3 Mid Price & Rolling Buffer)
* На каждом входящем тике стакана рассчитывается неспуфируемая средняя цена первых 3 уровней:
  $$P_{mid} = \frac{\frac{a_1 + a_2 + a_3}{3} + \frac{b_1 + b_2 + b_3}{3}}{2}$$
* Значения буферизируются в `collections.deque` с меткой монотонного времени `time.monotonic()`.
* При каждом замере устаревшие тики ($t < t_{now} - \text{buffer\_window\_sec}$) отсекаются.
* Вычисляется арифметическое среднее буфера $\bar{P} = \frac{1}{N}\sum P_i$.
* Проверяется относительное отклонение текущей цены от среднего:
  $$\delta = \frac{|P_{current} - \bar{P}|}{\bar{P}} \le \text{max\_static\_leg\_pct}$$
* Если $\delta > \text{max\_static\_leg\_pct}$, статичная нога пришла в движение — сигнал мгновенно бракуется (`TARGET_NOT_STATIC`), и тяжелая глубокая математика скипается.
